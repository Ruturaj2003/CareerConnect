<template>
  <main class="flex h-screen flex-col">
    <section class="pb-20 pt-10">
      <div class="grid grid-cols-12">
        <div class="col-span-1 col-start-1"></div>
        <div class="col-span-5 col-start-2">
          <the-headline></the-headline>
          <job-search-form></job-search-form>
        </div>
        <div class="col-span-5 col-start-7 flex justify-center self-center">
          <img
            src="https://upload.wikimedia.org/wikipedia/commons/thumb/9/95/Vue.js_Logo_2.svg/2367px-Vue.js_Logo_2.svg.png"
            alt=""
            class="h-80 w-80 object-contain"
          />
        </div>
        <div class="col-span-1 col-start-12"></div>
      </div>
    </section>

    <spot-light class="flex justify-center pb-16">
      <!-- We can use desturcting syntax of ES6 ie: ={img , title , description} -->
      <template v-slot:default="slotProps">
        <router-link
          to="/jobs/results"
          class="mx-5 flex h-96 w-72 flex-col rounded-lg border border-brand-gray-1 bg-brand-gray-2"
        >
          <img :src="slotProps.spotlight.img" class="rounded-t-md object-contain" />
          <div class="mt-3 h-48 px-6 py-4">
            <h3 class="text-lg font-medium">{{ slotProps.spotlight.title }}</h3>

            <p class="mt-3 text-sm">{{ slotProps.spotlight.description }}</p>
          </div>
          <router-link to="/jobs/results" class="px-6 pb-4 text-sm text-brand-blue-1"
            >See Jobs</router-link
          >
        </router-link>
      </template>
    </spot-light>
  </main>
</template>

<script setup lang="ts">
import TheHeadline from './TheHeadline.vue';

import JobSearchForm from './JobSearchForm.vue';
import SpotLight from '@/components/jobSearch/SpotLight.vue';
</script>
